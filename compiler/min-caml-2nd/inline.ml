...
(*
let rec size = function (*sizeによりinliningを止めるのは徹底的でないので却下*)
...
let rec isleaf = function
  | IfEq(_, _, e1, e2) | IfLE(_, _, e1, e2)
  | Let(_, e1, e2) | LetRec({ body = e1 }, e2) -> isleaf e1 && isleaf e2
  | LetTuple(_, _, e) -> isleaf e
  | App _ -> false
  | _ -> true
...
let rec g env = function (* インライン展開ルーチン本体 (caml2html: inline_g) *)
  ...
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2) -> (* 関数定義の場合 (caml2html: inline_letrec) *)
      let env = if not (isleaf e1) (*size e1 > 50 *)(*!threshold*) then env else M.add x (yts, e1) env in
