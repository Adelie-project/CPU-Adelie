...
and g' oc = function (* 各命令のアセンブリ生成 (caml2html: emit_gprime) *)
  ...
  | NonTail(_), Nop -> ()
  | NonTail(x), Set(i) -> Printf.fprintf oc "\taddi\t%s, 0, %d" x i(*"\tset\t%d, %s\n" i x*)
  | NonTail(x), SetL(Id.L(y)) -> Printf.fprintf oc "setlってなに"(*"\tset\t%s, %s\n" y x*)(*なんだこれ?*)
  | NonTail(x), Mov(y) when x = y -> ()
  | NonTail(x), Mov(y) -> Printf.fprintf oc "\tadd\t%s, 0, %s\n" y x(*"\tmov\t%s, %s\n" y x*)
  ...
  | NonTail(a), CallCls(x, ys, zs) ->
      g'_args oc [(x, reg_cl)] ys zs;
      let ss = stacksize () in
      Printf.fprintf oc "\tsw\t%s, %s, %d\n" reg_sp reg_ra (ss - 4);(*"\tst\t%s, [%s + %d]\n" reg_ra reg_sp (ss - 4);*)
      Printf.fprintf oc "\tlw\t%s, %s, 0\n" reg_sw reg_cl;(*"\tld\t[%s + 0], %s\n" reg_cl reg_sw;*)
      Printf.fprintf oc "\tcall\t%s\n" reg_sw;(*何だこれ*)
      Printf.fprintf oc "\tadd\t%s, %d, %s\t! delay slot\n" reg_sp ss reg_sp;(*"\tadd\t%s, %d, %s\t! delay slot\n" reg_sp ss reg_sp;*)(*かえてないけど,即値でやっててだめ*)
