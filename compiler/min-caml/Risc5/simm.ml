...
let rec g env = function (* 命令列の13bit即値最適化 (caml2html: simm13_g) *)
  ...
  | Let((x, t), Set(i), e) when -2048 <= i && i < 2048 ->(*我々の即値は12bitなので修正*)
  ...
  | Let(xt, SLL(y, C(i)), e) when M.mem y env -> (* for array access *)
      ...
      g env (Let(xt, Set((M.find y env) lsl i), e))(*(lsl)は中置記法の演算子,logic shift left*)
...
and g' env = function (* 各命令の13bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | SLL(x, V(y)) when M.mem y env -> SLL(x, C(M.find y env))*)(*slliがあるが,即値5bitなので如何せん*)(*そもそもSLLはvirtual.mlのarrayでしか発生しないのでここは通らないので無くてよい*)
